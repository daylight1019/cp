...
import { getIngredientList, updateSystems } from '../../../actions/settings';
import { connect } from 'react-redux';

class EditSystemScreen extends Component {
  constructor(props) {
    super(props);
  ...
  async componentDidMount() {
    ...
    this.system = this.props.route.params.system;
    console.log("Systems", JSON.stringify(this.system));
    this.setState({ systemName: this.system.name });
    this.setState({ systemPrice: this.system.saleprice.toString() });
    ...
    this.state.unitList = new Array(this.props.ingredientsInfo.length).fill('');
    this.state.extraList = new Array(this.props.ingredientsInfo.length).fill('');

    const tempItemState = this.state.itemsState.concat();
    const tempCheckState = this.state.checkState.concat();
    const tempUnitList = this.state.unitList.concat();
    const tempExtraList = this.state.extraList.concat();
    ...
    this.setState({ itemsState: tempItemState });
    this.setState({ checkState: tempCheckState });
    this.setState({ unitList: tempUnitList });
    this.setState({ extraList: tempExtraList });
  }
  ...
  onSave = async () => {
    ...
    await this.props.fetchUpdateSystems(sendParam);
                    ...
                    (<View>
                      <View style={{ flexDirection: 'row', marginLeft: 50, marginTop: 20, height: 25, width: 30 }}>
                        ...
                        <View style={styles.smallTextInput}>
                          <TextInput containerWidth={200} paddingBottom={0} paddingTop={0} fontSize={11} containerMaxHeight={50} onChangeText={text => this.setState({ unitList: this.state.unitList.map((x, i) => i == index ? text : x) })} value={this.state.unitList[index]}></TextInput>
                        </View>
                      </View>
                      <View style={{ flexDirection: 'row', marginLeft: 50, height: 20 }}>
                        <Text style={{ fontSize: 11 }}>Extra</Text>
                        <View style={styles.smallTextInput}>
                          <TextInput containerWidth={200} containerWidth={200} paddingBottom={0} paddingTop={0} fontSize={11} containerMaxHeight={50} onChangeText={text => this.setState({ extraList: this.state.extraList.map((x, i) => i == index ? text : x) })} value={this.state.extraList[index]}></TextInput>
                        </View>
                      </View>
                    </View>)}
                </View>
        ...
        <ScrollView style={{ flex: 1, backgroundColor: '#F5FCFF', }}>
          <View style={styles.container}>
            ...
            <TouchableOpacity onPress={() => this.toggleIngredient()} style={{ ...styles.buttonView, backgroundColor: this.state.ingredientOpen ? "rgb(236,151,31)" : "rgb(51,122,183)" }}>
              ...
              <Text style={styles.buttonText}>Ingredient ({this.state.checkState.filter(x=>x==true).length})</Text>
            </TouchableOpacity>
...
const mapDispatchToProps = (dispatch) => ({
  ...
  fetchUpdateSystems: (system) => dispatch(updateSystems(system)),
});
